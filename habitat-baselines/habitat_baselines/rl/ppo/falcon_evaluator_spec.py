...
class FALCONEvaluatorSpec(Evaluator):
    """
    ...
    def evaluate_agent(
        ...
        #csv load path
        file_path = 'evaluation/to_get_video.csv'
        print("csv load path:", file_path)
        df, episode_ids = load_csv_and_get_episode_ids(file_path)
        ...
        #do not eval prev evaled episodes ADD
        ...
        while (
            ...
            for i in range(envs.num_envs):
                if current_episodes_info[i].scene_id + " " +current_episodes_info[i].episode_id not in episode_ids:
            ...
            for i in range(n_envs):
                ...
                #if not not_done_masks[i].any().item():
                if not not_done_masks[i].any().item() and (current_episodes_info[i].scene_id + " " +current_episodes_info[i].episode_id in episode_ids):
                    # pbar.update()
                    ...
                    #csv update
                    episode_stats['episode_id'] = current_episodes_info[i].scene_id + " " +current_episodes_info[i].episode_id
                    print("update episode_stats['episode_id']", episode_stats['episode_id'])
                    update_csv(file_path, episode_stats)
...
def load_csv(file_path):
    """
    Load a CSV file. Create an empty DataFrame if the file doesn't exist.

    :param file_path: Path to the CSV file.
    :return: DataFrame containing the CSV data or an empty DataFrame if the file doesn't exist.
    """
...
def load_csv_and_get_episode_ids(file_path):
    """
    Load a CSV file and get unique values from the 'episode_id' column.
    Create an empty DataFrame if the file doesn't exist.

    :param file_path: Path to the CSV file.
    :return: A tuple containing the DataFrame and a set of unique episode_ids.
    """
    if os.path.exists(file_path):
        # Load the existing CSV file
        df = pd.read_csv(file_path)
        
        # Get unique values from the 'episode_id' column
        if 'episode_id' in df.columns:
            episode_ids = set(df['episode_id'].unique())
        else:
            episode_ids = set()
        
        return df, episode_ids
    else:
        # Return an empty DataFrame and an empty set if the file doesn't exist
        return pd.DataFrame(), set()

def update_csv(file_path, column_values):
    """
    Update the CSV file by adding a new row with specified column values.

    :param file_path: Path to the CSV file.
    :param column_values: Dictionary of column names and values to add.
    """
    df = load_csv(file_path)  # Load or create the CSV file

    # Convert column_values to a DataFrame
    new_row = pd.DataFrame([column_values])
