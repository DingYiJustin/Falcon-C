...
class ObstacleMap(BaseMap):
    ...
    def update_map(
        ...
        if update_obstacles:
            ...
            self._navigable_map = (1 - self._map.astype(np.uint8)).astype(bool)
    ...
    def get_ten_meter_data(self, tf_camera_to_episodic):
        agent_xy_location = tf_camera_to_episodic[:2, 3] #camera_position[:2] #tf_camera_to_episodic[:2, 3]
        
        current_angle= extract_yaw(tf_camera_to_episodic) #no - because need to turn backwards
        
        agent_pixel_location = self._xy_to_px(agent_xy_location.reshape(1, 2))[0]
        robot_x, robot_y = agent_pixel_location[::-1]
        # robot_x, robot_y = robot_x, -robot_y
        
        # Define the radius in pixels
        # radius_meters = 2.5
        radius_meters = 5
        radius_pixels = int(radius_meters * self.pixels_per_meter)
        
        
        # Get the shape of the explored area and map
        height, width = self.explored_area.shape

        # Create an output array filled with zeros with the required shape
        combined_area = np.zeros((2 * radius_pixels + 1, 2 * radius_pixels + 1, 2), dtype=np.float32)

        # Define the bounds of the area to extract
        x_start = max(robot_x - radius_pixels, 0)
        # x_end = min(robot_x + radius_pixels + 1, width)
        x_end = min(robot_x + radius_pixels + 1, height)
        y_start = max(robot_y - radius_pixels, 0)
        # y_end = min(robot_y + radius_pixels + 1, height)
        y_end = min(robot_y + radius_pixels + 1, width)

        # Extract the relevant slices from both maps
        # explored_slice = self.explored_area[x_start:x_end, y_start:y_end]
        # map_slice = self._map[x_start:x_end ,y_start:y_end]
        
        # Extract the relevant slices from both maps
        explored_slice = (self.explored_area[x_start:x_end, y_start:y_end].astype(np.uint8)) * 255 #for cv2 to process
        map_slice = (self._map[x_start:x_end ,y_start:y_end].astype(np.uint8)) * 255

        # Calculate the offsets for placing the slices in the combined area
        x_offset = radius_pixels - (robot_x - x_start)
        y_offset = radius_pixels - (robot_y - y_start)
        
        
        # Rotate the slices based on the current angle (considering x as height and y as width)
        center = (explored_slice.shape[0] // 2, explored_slice.shape[1] // 2)  # (width, height) for OpenCV
        rotation_matrix = cv2.getRotationMatrix2D(center, np.degrees(current_angle), 1.0)  # Angle in degrees

        # Rotate the slices
        rotated_explored_slice = (cv2.warpAffine(explored_slice, rotation_matrix, (explored_slice.shape[1], explored_slice.shape[0]))> 127).astype(np.float32)
        rotated_map_slice = (cv2.warpAffine(map_slice, rotation_matrix, (map_slice.shape[1], map_slice.shape[0]))> 127).astype(np.float32)

        # Place the extracted areas into the combined area
        combined_area[x_offset:x_offset + explored_slice.shape[0],
                    y_offset:y_offset + explored_slice.shape[1], 0] = rotated_explored_slice # explored_slice

        combined_area[x_offset:x_offset + map_slice.shape[0],
                    y_offset:y_offset + map_slice.shape[1], 1] = rotated_map_slice #map_slice
