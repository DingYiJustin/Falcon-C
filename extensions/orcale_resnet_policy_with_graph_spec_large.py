...
class PointNavResNetPolicyWithGraphSpecL(NetPolicy):
    def __init__(
        self,
        ...
        **kwargs,
        ...
        super().__init__(
            PointNavResNetWithGraphNetSpecL(
...
class ResNetEncoder(nn.Module):
    def __init__(
        ...
        self.visual_keys = [
            ...
            if len(v.shape) > 1 and k != ImageGoalSensor.cls_uuid and k != "oracle_humanoid_future_trajectory" and k != 'obstacle_map'and k != "oracle_humanoid_future_trajectory_map" and k != 'oracle_humanoid_future_trajectory_map_count'
        ]
        ...
        # print('self._n_input_channels, self.visual_keys',self._n_input_channels, self.visual_keys)
...
class SimpleResNetSameDim(nn.Module):
    def __init__(self, input_dim, num_layers):
        super(SimpleResNetSameDim, self).__init__()
        self.layers = self._make_layers(input_dim, num_layers)

    def _make_layers(self, input_dim, num_layers):
        layers = []
        for _ in range(num_layers-1):
            layers.append(BasicBlock(input_dim))
...
class PointNavResNetWithGraphNetSpecL(Net):
    ...
    def __init__(
        ...
        # print('EpisodicGPSSensor.cls_uuid ,observation_space.spaces', EpisodicGPSSensor.cls_uuid ,observation_space.spaces )
        if EpisodicGPSSensor.cls_uuid in observation_space.spaces:
            # print('EpisodicGPSSensor.cls_uuid in observation_space.spaces')
        ...
        if EpisodicCompassSensor.cls_uuid in observation_space.spaces:
            ...
            rnn_input_size += 32
            # print("EpisodicCompassSensor.cls_uuid in observation_space.spaces:")
        ...
        self.graph_pos_encoder = nn.Linear(2, 32)
        # self.graph_time_encoder = nn.Linear(1, 16)
        self.w_a = nn.Parameter(torch.randn(32, 32))
        self.w1 = nn.Parameter(torch.randn(32,128))
        self.w2 = nn.Parameter(torch.randn(128,128))
        self.w3 = nn.Parameter(torch.randn(128,128))
        self.w4 = nn.Parameter(torch.randn(128,256))
        self.w5 = nn.Parameter(torch.randn(256,256))
        self.graph_projector = nn.Linear(5* 256, self._hidden_size+ rnn_input_size)
                
        self.scaler_predictor = SimpleResNetSameDim((self._hidden_size+ rnn_input_size)*2, 4)
        
        # self.shift_predictor = SimpleResNetSameDim((self._hidden_size+ rnn_input_size)*2, 3)
        
        self.scaler_softmax = nn.Softmax(dim=2)
        
        # future_step = 5
        # self.temporal_steps = torch.arange(future_step, dtype=torch.float32).view(1, 1, future_step, 1)
        self.tanh = nn.Tanh()
    ...
    def compute_similarity_matrix(self, X): 
        A = torch.matmul(torch.matmul(X, self.w_a), X.permute(0, 1,3, 2)) # batch, seq_len * human + obstacles
        # expanded_mask = mask.unsqueeze(-1).unsqueeze(-1)
        # A[expanded_mask] = float('-inf')
        normalized_A = torch.nn.functional.softmax(A, dim=3)
        return normalized_A
    ...
    def forward(
        ...
        # map_input = self.pre_forward(observations)
        # print('observations.keys()',observations.keys())
        human_future_trajectory = observations["oracle_humanoid_future_trajectory"]
        human_future_trajectory = human_future_trajectory.to(torch.float32)
        # mask = (human_future_trajectory <= -99.9).any(dim=2).any(dim=2)
        human_future_trajectory = human_future_trajectory.permute(0, 2,1,3)
        # batch_size, num_human, future_step, _ = human_future_trajectory.shape
        
        # temporal_steps = self.temporal_steps.to(human_future_trajectory.device)
        # temporal_steps = self.graph_time_encoder(temporal_steps)
        
        
        node_features = self.graph_pos_encoder(human_future_trajectory)
        # node_features = torch.cat((node_features, temporal_steps.expand(batch_size, num_human, future_step, temporal_steps.shape[3])), dim=-1)
        # node_features = node_features.reshape(node_features.shape[0], node_features.shape[1]*node_features.shape[2], node_features.shape[3])
        # print("human_future_trajectory.shape",human_future_trajectory.shape)
        normalized_A = self.compute_similarity_matrix(node_features)
        # print("normalized_A.shape", normalized_A.shape)
        
        h1 = torch.nn.functional.relu(torch.matmul(torch.matmul(normalized_A, node_features), self.w1))
        h2 = torch.nn.functional.relu(torch.matmul(torch.matmul(normalized_A, h1), self.w2))
        h2 = torch.nn.functional.relu(torch.matmul(torch.matmul(normalized_A, h2), self.w3))
        h2 = h2+h1
        h2 = torch.nn.functional.relu(torch.matmul(torch.matmul(normalized_A, h2), self.w4))
        graph_feature = torch.nn.functional.relu(torch.matmul(torch.matmul(normalized_A, h2), self.w5))
        graph_feature = graph_feature+h2
        # print("graph_feature.shape", graph_feature.shape)
        final_graph_feature = torch.mean(graph_feature, dim=2).squeeze(dim=1)
        # print("final_graph_feature.shape", final_graph_feature.shape)
        final_graph_feature = final_graph_feature.reshape(node_features.shape[0], -1 )
        # print("final_graph_feature.shape1", final_graph_feature.shape)
        final_graph_feature = self.tanh(self.graph_projector(final_graph_feature)) * 6
        ...
        if not self.is_blind:
            ...
            if (  # noqa: SIM401
                PointNavResNetWithGraphNetSpecL.PRETRAINED_VISUAL_FEATURES_KEY
                in observations
            ):
        ...
        scalers = self.scaler_predictor(torch.cat([out, final_graph_feature],dim=1))
    
        scalers = scalers.reshape(scalers.shape[0], scalers.shape[1]//2, 2)
        scalers = self.scaler_softmax(scalers)
        scalers = torch.clamp(scalers, min=0.2, max=0.8)
        # scalers = torch.clamp(scalers, min=0.35, max=0.65)
        
        scaler_out, scaler_final_graph = scalers[:, :, 0], scalers[:,:, 1]
        # print("scaler_final_graph", scaler_final_graph)
        # print("final_graph_feature", final_graph_feature.mean(),final_graph_feature.max(), final_graph_feature.min())
        # print("out", out.mean(), out.max(), out.min())
        
        weighted_out = out * scaler_out  # Shape: (batch_size, n)
        weighted_final_graph_feature = final_graph_feature * scaler_final_graph
        out = weighted_out + weighted_final_graph_feature
        
        # out = (out + final_graph_feature)/2
