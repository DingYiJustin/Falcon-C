...
class PointNavResNetPolicyWithGraphInjectFilter(NetPolicy):
    def __init__(
        self,
        ...
        **kwargs,
        ...
        super().__init__(
            PointNavResNetWithGraphNetInjectFilter(
...
class PointNavResNetWithGraphNetInjectFilter(Net):
    ...
    def __init__(
        ...
        # self.graph_projector1 = nn.Linear(256, 128)
    ...
    def forward(
        ...
        # print('human_future_trajectory.shape1', human_future_trajectory.shape)
        mask = (human_future_trajectory <= -99.9).any(dim=1).any(dim=2)
        print('mask.shape', mask.shape)
        mask = mask.unsqueeze(1).unsqueeze(-1).expand(-1, 5, -1, 2)
        print('mask1.shape', mask.shape)
        # mask1 = (human_future_trajectory <= -99.9).any(dim=2)
        # print('mask1.shape', mask1.shape)
        shape = human_future_trajectory.shape
        human_future_trajectory = human_future_trajectory[mask].reshape(shape[0],shape[1],-1,shape[3])
        ...
        graph_feature = torch.mean(graph_feature, dim=2)
        
        # final_graph_feature = final_graph_feature.reshape(node_features.shape[0], -1 )
        final_graph_feature = graph_feature.reshape(node_features.shape[0], -1 )
        # final_graph_feature = self.bn1(self.graph_projector(final_graph_feature))
        final_graph_feature = self.layer_norm2(self.graph_projector(final_graph_feature))
        # final_graph_feature = self.graph_projector1(final_graph_feature)
        ...
        if not self.is_blind:
            ...
            if (  # noqa: SIM401
                PointNavResNetWithGraphNetInjectFilter.PRETRAINED_VISUAL_FEATURES_KEY
                in observations
            ):
