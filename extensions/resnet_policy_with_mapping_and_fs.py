...
class PointNavResNetPolicyWithMap(NetPolicy):
    def __init__(
        self,
...
class ResNetEncoder(nn.Module):
    def __init__(
        ...
        self.visual_keys = [
            ...
            if len(v.shape) > 1 and k != ImageGoalSensor.cls_uuid and k != "oracle_humanoid_future_trajectory" and k != 'obstacle_map'and k != "oracle_humanoid_future_trajectory_map"
        ]
...
class SimpleResNetBasicBlock(nn.Module):
    expansion = 1

    def __init__(self, in_channels, out_channels, stride=1, downsample=None):
        super(SimpleResNetBasicBlock, self).__init__()
...
class SimpleResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.avgpool(x)
        x = torch.flatten(x, 1)
        # x = self.fc(x) #don't need fc

        return x


def GetSimpleResnet18ForMap(input_dim):
    #Input → 7×7 Conv → MaxPool → [Stage 2–5 ResBlocks] → GAP → FC → Output
    return SimpleResNet(SimpleResNetBasicBlock, [2, 2, 2], input_dim)
        



class PointNavResNetWithMapNet(Net):
    ...
    def __init__(
        ...
        # add map module input size and future map module input size
        rnn_input_size += 128
        # rnn_input_size += 128
        ...
        self.map_encoder = GetSimpleResnet18ForMap(2)
        self.future_map_encoder = GetSimpleResnet18ForMap(5)
        # self.map_state_encoder = build_rnn_state_encoder(
        #     512,
        #     self._hidden_size,
        #     rnn_type=rnn_type,
        #     num_layers=num_recurrent_layers,
        # )
        # self.map_state_encoder = torch.nn.MultiheadAttention(embed_dim=2, num_heads=2, batch_first=True)
        # map_output_feature = 1250
        # self.relu = torch.nn.ReLU()
        # self.final_encoder = torch.nn.Linear(self._hidden_size*2, self._hidden_size)
    ...
    def forward(
        ...
        map_input = observations["obstacle_map"]
        future_map_input = observations["future_map"]
        ...
        if not self.is_blind:
            ...
            if (  # noqa: SIM401
                PointNavResNetWithMapNet.PRETRAINED_VISUAL_FEATURES_KEY
                in observations
            ):
        ...
        #append map observation
        # print(type(map_input))
        map_input = map_input.permute(0, 3, 1, 2)
        future_map_input = future_map_input.permute(0, 3, 1, 2)
        out_map = self.map_encoder(map_input)
        out_map_future = self.map_encoder(future_map_input)
        final_map = torch.add(out_map, out_map_future, alpha=0.1 ) #use 0.1 to decrease the init effect of out_map_future
        # print('out_map',out_map.shape)
        x.append(final_map)
        # x.append(future_map_input)
        # out_map = out_map.reshape(out_map.shape[0],-1)
