...
class PointNavResNetPolicyWithGraphInject(NetPolicy):
    def __init__(
        self,
        ...
        **kwargs,
        ...
        super().__init__(
            PointNavResNetWithGraphNetInject(
...
class PointNavResNetWithGraphNetInject(Net):
    ...
    def __init__(
        ...
        self.graph_projector = nn.Linear(5* 32, 128)
        self.bn1 = nn.BatchNorm1d(128)
        
        self.layer_norm = nn.LayerNorm([5,6,32])
        self.layer_norm1 = nn.LayerNorm([5,6,32])
        
        self.output_projector = nn.Linear(self._hidden_size+ rnn_input_size, self._hidden_size+ rnn_input_size - 128)
    ...
    def compute_similarity_matrix(self, X, mask): 
        A = torch.matmul(torch.matmul(X, self.w_a), X.permute(0, 1,3, 2)) # batch, seq_len * human + obstacles
        mask = mask.unsqueeze(1).unsqueeze(-1).expand(-1, 5, -1, 6)
        # print('a.shape', A.shape, mask.shape)
        A[mask] = -1e9
    ...
    def forward(
        ...
        mask = (human_future_trajectory <= -99.9).any(dim=1)
        print('mask.shape', mask.shape)
        mask1 = (human_future_trajectory <= -99.9).any(dim=2)
        print('mask1.shape', mask1.shape)
        # human_future_trajectory = human_future_trajectory
        ...
        node_features[mask.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, 5, 32)] = 0.
        # print("(node_features.shape",node_features.shape, mask.shape)
        ...
        # print("human_future_trajectory.shape",human_future_trajectory.shape)
        normalized_A = self.compute_similarity_matrix(node_features,mask)
        # print("normalized_A.shape", normalized_A.shape)
        
        h1 = node_features + torch.nn.functional.relu(torch.matmul(torch.matmul(normalized_A, self.layer_norm(node_features)), self.w1))
        graph_feature = h1 + torch.nn.functional.relu(torch.matmul(torch.matmul(normalized_A, self.layer_norm1(h1)), self.w2))
        ...
        final_graph_feature = self.bn1(self.graph_projector(final_graph_feature))
        ...
        if not self.is_blind:
            ...
            if (  # noqa: SIM401
                PointNavResNetWithGraphNetInject.PRETRAINED_VISUAL_FEATURES_KEY
                in observations
            ):
        ...
        out = torch.cat([out , final_graph_feature], dim=1) 
