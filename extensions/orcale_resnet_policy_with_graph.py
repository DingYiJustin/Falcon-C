...
class PointNavResNetPolicyWithGraph(NetPolicy):
    def __init__(
        self,
...
class PointNavResNetWithGraphNet(Net):
    ...
    def __init__(
        ...
        self.w2 = nn.Parameter(torch.randn(128,256))
        self.graph_projector = nn.Linear(5* 256, self._hidden_size+ rnn_input_size)
        
        self.scaler_predictor = nn.Linear((self._hidden_size+ rnn_input_size)*2, 2)
        self.scaler_softmax = nn.Softmax(dim=1)
    ...
    def forward(
        ...
        graph_feature = torch.nn.functional.relu(torch.matmul(torch.matmul(normalized_A, h1), self.w2))
        ...
        final_graph_feature = self.tanh(self.graph_projector(final_graph_feature))
        # final_graph_feature = self.tanh(self.graph_projector(final_graph_feature)) *3

        
        # final_graph_feature = (self.sigmoid(final_graph_feature)-0.5) * 10
        # print("final_graph_feature.shape2", final_graph_feature.shape)
        
        # print('map_input.dtype', map_input.dtype)
        # print('map_input.shape', map_input.shape)
        ...
        if not self.is_blind:
            ...
            if (  # noqa: SIM401
                PointNavResNetWithGraphNet.PRETRAINED_VISUAL_FEATURES_KEY
                in observations
            ):
        ...
        scalers = self.scaler_softmax(self.scaler_predictor(torch.cat([out, final_graph_feature],dim=1)))
        ...
        weighted_out = out * scaler_out.unsqueeze(1)  # Shape: (batch_size, n)
        weighted_final_graph_feature = final_graph_feature * scaler_final_graph.unsqueeze(1)
        out = weighted_out + weighted_final_graph_feature
        
        # out = out + final_graph_feature
